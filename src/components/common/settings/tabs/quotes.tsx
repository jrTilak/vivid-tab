...
import { DeleteIcon, Trash2Icon, XIcon } from 'lucide-react'
...
type QuotesCategories = SettingsConfig["quotes"]["categories"]

const QuotesSettings = () => {
  const { settings: { quotes }, setSettings } = useSettings()
        ...
        >
          {
            isLoaded ?
              categories.map(({ name, slug }) => (
                <div key={slug} className='flex items-center space-x-2 text-muted-foreground'>
                  <Checkbox
                    id={slug}
