	...
	const onDragEnd = (event: DragEndEvent) => {
		...
		setSettings((prevSettings) => {
			...
			if (from.toString().startsWith("empty")) {
				delete obj.widgets.layout[current.index];
			}

			if (to.toString().startsWith("empty")) {
				delete obj.widgets.layout[activeCurrent.index];
			}
	...
	return (
		<div className="flex flex-col gap-6 p-4">
			<div className="grid gap-4 grid-cols-4">
				{(Object.keys(COMPONENTS) as Array<keyof typeof COMPONENTS>).map(
					(key) => (
								...
								onCheckedChange={(val) => {
									if (val) {
										...
										if (key === "bookmarks") {
											...
											const usedIndex = Object.keys(layout).map((k) =>
												Number(k),
											);
