...
/**
 * Maps local-file bookmarks to bundled type icons without touching web URLs.
 * Unknown, extensionless, and malformed file URLs use the generic broken-file
 * icon so bookmark rendering never throws.
 *
 * @example
 * ```ts
 * getFileIcon("file:///home/user/report.pdf");
 * // chrome-extension://.../assets/svg/pdf-file.svg
 * ```
 *
 * @param fileUrl - Bookmark URL to inspect.
 * @returns A bundled extension URL for local files, otherwise `null`.
 */
export const getFileIcon = (fileUrl: string): string | null => {
	...
	try {
		fileExtension = new URL(fileUrl).pathname.split(".").pop()?.toLowerCase();
	} catch {
